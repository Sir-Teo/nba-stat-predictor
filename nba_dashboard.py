...
class NBADashboard:
    ...
    def show_system_status(self):
        ...
        checks = [
            ...
            (
                "Models trained",
                all(model_status[stat]["trained"] for stat in model_status),
            ),
        ...
        try:
            ...
            for category_name, stat_col, avg_expr in categories:
                ...
                for i, (_, row) in enumerate(df.iterrows(), 1):
                    print(
                        f"   {i:2d}. {row['player_name']:25} {row['avg_stat']:5.1f} ({row['games']} games)"
                    )
    ...
    def suggest_next_actions(self):
        ...
        try:
            ...
            try:
                cursor.execute(
                    "SELECT COUNT(*) FROM predictions WHERE date >= date('now', '-1 days')"
                )
                recent_predictions = cursor.fetchone()[0]
            except:
                recent_predictions = 0
            ...
            if total_games < 100:
                ...
                suggestions.append(
                    "   Need more training data for reliable predictions"
                )
...
def main():
    ...
    parser.add_argument(
        "--db-path", default="data/nba_data.db", help="Path to SQLite database"
    )
    parser.add_argument(
        "--interactive", action="store_true", help="Run interactive menu"
    )
    parser.add_argument(
