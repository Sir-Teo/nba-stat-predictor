...
class AdvancedStatPredictor:
    ...
    def _evaluate_model(self, X: pd.DataFrame, y: pd.Series, cv) -> Dict[str, float]:
        ...
        metrics = {
            ...
            'accuracy_within_10pct': np.mean(np.abs(y_test - y_pred_test) / np.maximum(y_test, 1e-8) <= 0.1) * 100,
            'accuracy_within_20pct': np.mean(np.abs(y_test - y_pred_test) / np.maximum(y_test, 1e-8) <= 0.2) * 100
        }
    ...
    def _calculate_confidence_intervals(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, float]:
        ...
        with tqdm(range(n_bootstrap), desc="Bootstrap CI", ncols=60, 
                  bar_format="{desc}: {n_fmt}/{total_fmt}", leave=False) as pbar:
            for i in pbar:
                # Bootstrap sample
...
class LightGBMStatPredictor(AdvancedStatPredictor):
    ...
    def _optimize_hyperparameters(self, X: pd.DataFrame, y: pd.Series, cv) -> lgb.LGBMRegressor:
...
class NeuralNetworkStatPredictor(AdvancedStatPredictor):
    ...
    def _get_param_space(self):
        """Get parameter space for hyperparameter optimization."""
        return {
            'hidden_layer_sizes': Categorical([(50,), (100,), (100, 50), (100, 50, 25), (200, 100, 50)]),
...
class AdvancedEnsembleStatPredictor(AdvancedStatPredictor):
    ...
    def __init__(self, stat_type: str, model_version: str = "ensemble_v2.0"):
        ...
        if CATBOOST_AVAILABLE:
            self.base_models['cat'] = cb.CatBoostRegressor(random_seed=42, verbose=False)
    ...
    def train(self, X: pd.DataFrame, y: pd.Series, optimize_hyperparams: bool = True) -> Dict[str, float]:
        ...
        successful_models = 0
        with tqdm(self.base_models.items(), desc="Base Models", ncols=60, 
                  bar_format="{desc}: {n_fmt}/{total_fmt}", leave=False) as pbar:
        ...
        tqdm.write(f"   ✅ Successfully trained {successful_models}/{len(self.base_models)} base models")
    ...
    def _generate_stacking_features(self, X: pd.DataFrame, y: pd.Series, cv) -> np.ndarray:
        ...
        with tqdm(total=total_iterations, desc="CV Folds", ncols=60, 
                  bar_format="{desc}: {n_fmt}/{total_fmt}", leave=False) as pbar:
            for fold, (train_idx, val_idx) in enumerate(cv_splits):
                ...
                for i, (name, model) in enumerate(self.base_models.items()):
                    ...
                    try:
                        ...
                        tqdm.write(f"     ⚠️  Failed to generate stacking features for {name} in fold {fold+1}: {e}")
                        stacking_features[val_idx, i] = y_train.mean()  # Fallback to mean
                    pbar.update(1)
...
class ModelManager:
    ...
    def create_predictor(self, stat_type: str, model_type: str = "ensemble") -> AdvancedStatPredictor:
        ...
        if model_type == "random_forest":
            ...
            raise ValueError(f"Unknown model type: {model_type}. Available: random_forest, xgboost, lightgbm, catboost, neural_network, ensemble")
        
        return predictor
    
    def train_model(self, stat_type: str, training_data: pd.DataFrame, 
                   model_type: str = "ensemble", optimize_hyperparams: bool = True) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"Trained {stat_type} model - Test MAE: {metrics.get('test_mae', 0):.2f}, "
            ...
            if model_type != "random_forest":
                ...
                return self.train_model(stat_type, training_data, "random_forest", False)
    ...
    def _store_model_performance(self, stat_type: str, model_version: str, 
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (model_version, stat_type, mae, rmse, r2, datetime.now().isoformat(), sample_size))
