...
class TonightPredictor:
    ...
    def get_tonights_predictions(self) -> pd.DataFrame:
            ...
            for player_id, player_name in all_players:
                try:
                    ...
                    if features_df.empty:
                        logger.warning(f"No features available for player {player_name}")
                        continue
        ...
        if all_predictions:
            ...
            logger.info(f"Generated predictions for {len(final_predictions)} player-games")
    ...
    def _store_predictions(self, game_id: str, player_id: int, player_name: str,
                          game_date: str, predictions_df: pd.DataFrame) -> None:
        ...
        for stat_type in self.stat_types:
            ...
            if predicted_col in predictions_df.columns:
                ...
                confidence = predictions_df[confidence_col].iloc[0] if confidence_col in predictions_df.columns else 0.5
    ...
    def update_predictions_with_results(self, game_date: str = None) -> None:
        ...
        for game_id, player_id, stat_type in predictions_to_update:
            ...
            actual_df = pd.read_sql_query(actual_query, conn, params=(game_id, player_id))
        ...
        conn.close()
        logger.info(f"Updated {len(predictions_to_update)} predictions with actual results")
