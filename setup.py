...
def test_api_connection():
    ...
    try:
        ...
        if not test_players.empty:
            logger.info(f"✅ NBA API connection successful. Found {len(test_players)} players")
            return True
        else:
...
def run_quick_demo(sample_size=10):
    ...
    try:
        ...
        cmd = [sys.executable, "main.py", "collect-data", "--players-limit", str(sample_size)]
...
def main():
    ...
    parser.add_argument('--skip-demo', action='store_true', 
                       help='Skip the demo run (faster setup)')
