...
class NBABacktester:
    ...
    def run_season_backtest(
        ...
        train_cutoff = datetime.strptime(season_start, "%Y-%m-%d") + timedelta(
            days=train_months * 30
        )
        ...
        if len(players_with_data) < 20:
            logger.warning(
                f"Limited players with data: {len(players_with_data)}. Running with available data."
            )
            if len(players_with_data) < 1:
                raise ValueError(
                    f"Insufficient players with data: {len(players_with_data)}"
                )
        ...
        for stat_type in self.stat_types:
            try:
                ...
                logger.info(
                    f"Trained {stat_type} model - MAE: {metrics['val_mae']:.2f}"
                )
    ...
    def _get_players_with_sufficient_data(
        self, start_date: str, end_date: str, min_games: int
    ) -> List[int]:
    ...
    def _generate_test_predictions(
        self, players_list: List[int], start_date: str, end_date: str
    ) -> pd.DataFrame:
        ...
        for player_id in players_list:
            ...
            for i, (_, game) in enumerate(player_games.iterrows()):
                ...
                try:
                    ...
                    logger.warning(
                        f"Error predicting for player {player_id} on {game_date}: {e}"
                    )
                    continue
    ...
    def _calculate_accuracy_metrics(self, predictions_df: pd.DataFrame) -> Dict:
        ...
        for stat_type in self.stat_types:
            ...
            if (
                predicted_col in predictions_df.columns
                and actual_col in predictions_df.columns
            ):
    ...
    def print_backtest_results(self, results: Dict) -> None:
        ...
        for stat_type in self.stat_types:
            if stat_type in results["training_metrics"]:
                ...
                print(
                    f"{stat_type.upper():<4} {metrics['train_mae']:<10.2f} "
        ...
        print("-" * 60)
        print(
            f"{'Stat':<4} {'Test MAE':<9} {'Test R²':<8} {'Baseline':<9} {'Improve':<8} {'Samples':<8}"
        )
        print("-" * 55)
        ...
        for stat_type in self.stat_types:
            if (
                stat_type in results["test_metrics"]
                and results["test_metrics"][stat_type]["sample_size"] > 0
            ):
                total_improvements.append(
                    results["test_metrics"][stat_type]["improvement_pct"]
                )
    ...
    def create_backtest_visualizations(
        ...
        fig.suptitle(
            f"NBA Stat Predictor Backtest Results - {results['season']}", fontsize=16
        )
        ...
        for idx, stat_type in enumerate(self.stat_types):
            ...
            if (
                ...
                if len(valid_data) > 0:
                    # Scatter plot
                    ax.scatter(
                        valid_data[actual_col],
                        valid_data[predicted_col],
                        alpha=0.6,
                        s=20,
                    )
                    ...
                    min_val = min(
                        valid_data[actual_col].min(), valid_data[predicted_col].min()
                    )
                    max_val = max(
                        valid_data[actual_col].max(), valid_data[predicted_col].max()
                    )
    ...
    def run_rolling_backtest(
        self, season: str = "2023-24", window_days: int = 30, step_days: int = 7
    ) -> Dict:
        """
...
def main():
    ...
    try:
        ...
        results = backtester.run_season_backtest(
            season="2023-24", train_months=3, min_games_per_player=15
        )
